...
// useProductStock hook per cart line — hooks can't be called inside a
...
const CartLineItem = ({
  ...
  const { stock, loading: stockLoading } = useProductStock(item.product.id);
  const id = getItemId(item);
  const flatMax = getMaxQuantity(item.product);
  ...
  if (stock !== null && stock <= 0) {
    limitMessage = 'Out of stock';
  } else if (atMax && stock !== null && stock < flatMax) {
    limitMessage = `Only ${effectiveMax} left in stock`;
  } else if (atMax) {
    limitMessage = `Max ${effectiveMax} per order`;
  }
          ...
          <div className="flex-grow">
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-1">
                ...
                {limitMessage && (
                  <p className="text-xs font-medium text-destructive">{limitMessage}</p>
                )}
              </div>
